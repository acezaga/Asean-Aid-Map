...
angular.module('sbAdminApp').controller('createProjectSummaryController', function($scope, $rootScope, $http, $location, projectService, accountService, Authentication, toaster, $timeout){
    ...
    $scope.getOnhand = function(resourceType, resourceDescription){
        ...
        if($scope.project.resources[projectResIndex]) {
            if ($scope.project.resources[projectResIndex].type == 'Money') {
                result = (parseInt($scope.project.resources[projectResIndex].inhand) + parseInt($scope.project.budget.amount)) - parseInt(resourceUsage);
            }
            else {
                result = parseInt($scope.project.resources[projectResIndex].inhand) - parseInt(resourceUsage);
            }
        }
        else if(resourceType == 'Money'){
            result = parseInt($scope.project.budget.amount) - parseInt(resourceUsage);
        }
    ...
    $scope.saveChanges = function(){
        ...
        if($scope.activitySet.activityResources.length < 1){
            toaster.pop('error', 'Summary not saved', 'Activities should not be set null.');
        }
        else {
    ...
    $scope.checkIfDelete = function(index, activity){
        console.log('check if delete index: '+index);

        if(index >= 0 && index < activity.resources.length - 1){
            return true;
        }
    ...
    $scope.removeResourceOnActivity = function(activity, index){
        activity.resources.splice(index, 1);
    }
});
...
angular.module('sbAdminApp').controller('projectSummaryController', function($scope, $rootScope, $http, $location, projectService, accountService, Authentication){
    ...
    $scope.printSummaryReport = function(){
        ...
        var resourceBody = [];
        resourceBody.push(['Resources', 'Quantity Used', 'Quantity Remaining']);
        for(i = 0; i < $scope.resourcesUseRemain.length; i++){
            var resource = $scope.resourcesUseRemain[i];
            var remaining = $scope.getAllRemainingRes(resource);

            var data = [resource.type+"("+resource.description+")", resource.used, remaining];
            resourceBody.push(data);
        }
    ...
    $scope.usedAndRemainingRes = function(){
        $scope.summaryReport.activities.forEach(function(activity){
            activity.resources.forEach(function(resource){
                var index = $scope.resourcesUseRemain.findIndex(function(element){
                    return element.type == resource.type && element.description == resource.description;
                });

                if(index > -1){
                    $scope.resourcesUseRemain[index].used = parseInt($scope.resourcesUseRemain[index].used) + parseInt(resource.qty);
                }
                else{
                    $scope.resourcesUseRemain.push({type: resource.type, description: resource.description, used: resource.qty});
                }
            });
        });
    }

    $scope.getAllRemainingRes = function(resource){
        var index = $scope.project.resources.findIndex(function(element){
            return element.type == resource.type && element.description == resource.description;
        });

        if(resource.type == 'Money'){
            if(index > -1){
                return parseInt($scope.project.resources[index].inhand) + parseInt($scope.project.budget.amount) - parseInt(resource.used);
            }
            else{
                return parseInt($scope.project.budget.amount) - parseInt(resource.used);
            }
        }

        if(index > -1){
            return parseInt($scope.project.resources[index].inhand) - parseInt(resource.used);
        }
    }
...
angular.module('sbAdminApp').controller('performanceReport', function($scope, $rootScope, $http, $location, accountService, projectService, Authentication){
    ...
    $scope.getPerformanceStat = function() {
        projectService.getAllPerformanceStat($rootScope.account._id, $scope.year).then(function (response) {
            if (response.data.status) {
                ...
                console.log("performanceReport: "+JSON.stringify($scope.performanceReport));
                ...
                totalScore = totalScore/$scope.performanceReport.length;
    ...
    $scope.seeEvaluation = function(index){
        ...
        $scope.currentPerformanceReport.arrayOfComments = $scope.currentPerformanceReport.comment.split("::");
    ...
    function getKeyDetails(key){
        ...
        switch (key){
            case 'venue':
                return "Appropriateness and capacity of the venue where the project is held.";
            case 'speaker':
                return "Effectiveness of project speaker.";
            case 'time':
                return "Time allotment for the project scope.";
            case 'utilization':
                return "Utilization or allocation of project resources.";
            case 'equipments':
                return "Usage of project utilities and equipments.";
            case 'purpose':
                return "Significance of purpose or advocacy of the project.";
            case 'appeal':
                return "Appeal to potential collaborators and donors.";
            case 'effort':
                return "Overall effort exerted to the project.";
            case 'organization':
                return "Overall organization of the project regarding people involved and program flow.";
        }
        return details;
    }
});
...
function getResources(scope){
    ...
    if(scope.availResourcesList.indexOf('Money') < 0){
        if(scope.project.budget.amount > 0){
            scope.availResourcesList.push('Money');
        }
    }
    //}
}