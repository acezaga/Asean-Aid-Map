    ...
    .controller('projectProfileController', function($scope, $rootScope, socketService, Authentication, projectService, accountService, $location, toaster){
                ...
                if(parameters.name){
                    ...
                    projectService.getProject(parameters.name).then(function(response){
                        if(response.data.status){
                            ...
                            $scope.getActivityStatusProgress();
       ...
       /* };*/
        ...
        $scope.addRecipient = function(data) {
            ...
            if($scope.searchedUser.length > 0){
                searchedIndex = $scope.searchedUser.findIndex(function (account) {
                    return account._id == data._id;
                });
            }
        ...
        $scope.checkIfSearchTagged = function(data){
            var index = $scope.tags.findIndex(function(tag){
                            return tag.account._id == data._id;
                        });
        ...
        $scope.searchForUser = function(){
            console.log('search parameter: '+$scope.search.searchedUserField);
            accountService.getSearchUser($scope.search.searchedUserField).then(function(response){
                if(response.data.status){
        ...
        $scope.checkIfSearchNull = function(){
            if(!$scope.search.searchedUserField){
                $scope.searchedUser = [];
            }
        }
        ...
        $scope.onClickOrgName = function(username){
            //$rootScope.selectedAccount = account;
            $location.url('/dashboard/orgprofile?name='+username);
            //$state.reload();
            //$state.go('dashboard.orgprofile');
        };

        $scope.getActivityStatusProgress = function(){
            projectService.getProjectSummaryById($rootScope.selectedProject._id).then(function(response){
                if(response.data.status){
                    $scope.hasActivity = true;

                    $scope.projectSummary = response.data.summary;

                    $scope.activityCount = $scope.projectSummary.activities.length;
                    $scope.finishedActivity = 0;

                    $scope.projectSummary.activities.forEach(function(activity){
                        if(activity.finished){
                            $scope.finishedActivity++;
                        }
                    });

                    $scope.activityPercentage = ($scope.finishedActivity/$scope.activityCount)*100;
                }
                else{
