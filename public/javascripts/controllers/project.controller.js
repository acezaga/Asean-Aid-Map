   ...
   //    collaborators: []};
    ...
    $scope.saveChanges = function(){
        ...
        if($scope.project.startDate > $scope.project.endDate){
            return toaster.pop('error', 'Date error', 'Start date must not be greater than End date!');
        }
        ...
        projectService.createProject($scope.project).then(function(response){
            if(response.data.status) {
                ...
                var projconnInfo = {};
                projconnInfo.fromId = $rootScope.account._id;
                projconnInfo.fromName = $rootScope.account.orgname;
                projconnInfo.connections = $rootScope.account.connections;
                projconnInfo.projectId = $scope.createdProject._id;
                projconnInfo.projectName = $scope.createdProject.title;

                toaster.pop('success', 'Creation Successful', 'Project successfully created!');
                socketService.emit('new-project', projconnInfo, function(data){
                ...
                if(response.data.message){
                    response.data.message.forEach(function(mess){
                        toaster.pop('error', "Project not created", mess.msg);
                    })
                }
            }
        }, function(err){
    ...
    //$scope.checkForStartDate = function(){
    //    if($scope.project.startDate < new Date()){
    //        $scope.project.startDate = null;
    //        return toaster.pop('error', 'Date error', 'Start date must not be less than the date today');
    //    }
    //}

    $scope.checkForEndDate = function(){
        if($scope.project.startDate > $scope.project.endDate){
    ...
    $scope.saveChanges = function(){
        ...
        projectService.saveChanges($scope.project).then(function(response){
            if(response.data.status) {
                ...
                if(response.data.message){
                    response.data.message.forEach(function(mess){
                        toaster.pop('error', "Changes were not applied", mess.msg);
                    })
                }
            }
        }, function(err){
