            ...
            if(response.data.status) {
                ...
                if(response.data.message){
                    response.data.message.forEach(function(mess){
                        toaster.pop('error', "Account was not created", mess.msg);
                    });
                }
...
angular.module('sbAdminApp').controller('editProfileController', function($scope, $rootScope, $state, accountService, fieldService, Authentication, toaster, $timeout, $location){
            ...
            if(response.data.status){
                ...
                if(response.data.message){
                    if(response.data.message.constructor === Array) {
                        ...
                        toaster.pop('error', "Changes were not applied", response.data.message);
                    }
                }
            }
        });
    ...
    $scope.cancelEdit = function(){
        $location.url('/dashboard/orgprofile?name='+$scope.userDetails.username);
    }
});

angular.module('sbAdminApp').controller('accountSettingsController', function($scope, $rootScope, $state, accountService, fieldService, Authentication, toaster, $timeout, $location){
    Authentication.authentication();
    ...
    $scope.cancelEdit = function(){
        $location.url('/dashboard/orgprofile?name='+$scope.accountSettings.username);
    }
});
