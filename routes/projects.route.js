...
router.post('/create', function(req,res,next){
    ...
    req.checkBody('title', 'Project Title is Required').notEmpty();
    req.checkBody('address', 'Project Address is Required').notEmpty();
    req.checkBody('startDate', 'Project Start Date is Required').notEmpty();
    req.checkBody('endDate', 'Project Start Date is Required').notEmpty();
    req.checkBody('sector', 'Project Start Date is Required').notEmpty();
    req.checkBody('contacts', 'Project Start Date is Required').notEmpty();
    req.checkBody('resources', 'Project Start Date is Required').notEmpty();
    ...
    project.findOne({
        ...
        if (foundProject) {
            ...
            newProject.save(function(err, newProject){
                ...
                return res.send({status: true, project: newProject});
            });
        }
    });
...
router.put('/update', function(req, res, next){
    ...
    req.checkBody('project.title', 'Project Title is Required').notEmpty();
    req.checkBody('project.address', 'Project Address is Required').notEmpty();
    req.checkBody('project.startDate', 'Project Start Date is Required').notEmpty();
    req.checkBody('project.endDate', 'Project End Date is Required').notEmpty();
    req.checkBody('project.sector', 'Project Sector is Required').notEmpty();
    req.checkBody('project.contacts', 'Project Contacts is Required').notEmpty();
    req.checkBody('project.resources', 'Project Resources is Required').notEmpty();
    ...
    summary.findOne({project: summaryDetails.project}, function(err, foundSummary){
        ...
        if(!foundSummary){
            ...
            newSummary.save(function(err, savedSummary){
                return res.send({status: true, summary: savedSummary});
            });
        }
        else {
...
router.post('/savePerformanceReport', function(req, res, next){
    ...
    performance.findOne({project: report.project, evaluator: report.evaluator}, function(err, foundPerformance){
        ...
        performanceReport.comment = report.comment;
...
router.post('/allProjectSummaryByYear', function(req, res, next){
   ...
   project.find({$or: [{creator: id}, {collaboration: id}], startDate: {$gte: year}}, '_id', function(err, foundProject){
      if(err){}
      if(foundProject.length < 1){
          return res.send({status: false});
      }
        ...
        performance.find({project: {$in: projects}}, function(err, foundPerformance){
            ...
            for(i = 0; i < foundPerformance.length; i++){
                ...
                if(performanceByProject[foundPerformance[i].project]) {
                    ...
                    performanceByProject[foundPerformance[i].project].comment += "::" + foundPerformance[i].comment;
                }
                else{
                    performanceByProject[foundPerformance[i].project] = foundPerformance[i];
                    //console.log("added array of comments");
                    //performanceByProject[foundPerformance[i].project].arrayOfComments = [];
                    //performanceByProject[foundPerformance[i].project].arrayOfComments.push(foundPerformance[i].comment);
                    //console.log("comments: "+performanceByProject[foundPerformance[i].project].arrayOfComments);
                }
            }

            console.log("performance with comments: "+JSON.stringify(performanceByProject));
...
router.get('/projectSummaryById/:id', function(req, res, next){
    var projectId = req.params.id;

   summary.findOne({project: projectId}, function(err, foundSummary){
      if(err){
          return res.send({status: false});
      }
      if(!foundSummary){
