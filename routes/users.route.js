...
module.exports = function(passport) {
    ...
    router.put('/saveChanges/', function(req, res, next){
        ...
        console.log("body: " + JSON.stringify(req.body.account));

        var updatesDetail = req.body.account;

        req.checkBody('account.orgname', 'Organization name is required').notEmpty();
        req.checkBody('account.sectors', 'Organization Sector is required').notEmpty();
        req.checkBody('account.resources', 'Organization Resources is required').notEmpty();
        req.checkBody('account.country', 'Organization Country is required').notEmpty();
        ...
        user.findOne({_id: updatesDetail._id}, function(err, foundUser){
            ...
            foundUser.representative = updatesDetail.representative;
    ...
    router.post('/suggestedAccountsToProject', function(req, res, next){
        ...
        user.find({'country': criteria.location, resources: {$elemMatch: {type: {$in: resourcesType}}}, sectors: {$elemMatch: {type: {$in: sectorsType}}}}, function(err, foundUsers){
            if(err){
